...
// Shared by both localStorage (loadState) and Firestore (cloudSync) loading
// paths so a saved/synced blob from an older schema can never silently drop
// newer fields or crash on an old shape — one merge rule, two sources.
export function mergeState(fallback, saved) {
